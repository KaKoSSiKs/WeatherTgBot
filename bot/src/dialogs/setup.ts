...
async function promptLocation(ctx: Context, flow: FlowType) {
  await ctx.reply(
    flow === 'setup'
      ? 'Начнём с локации. Выберите город из списка или отправьте свою геопозицию через обычное сообщение.'
      : 'Где нужно присылать погоду? Выберите город или отправьте свою геопозицию через обычное сообщение.',
