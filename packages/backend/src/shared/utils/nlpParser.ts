/**
 * NLP Parser for Notification Commands
 * 
 * Парсит естественный язык для команд создания уведомлений.
 */

import { geocodeCity, DEFAULT_CITY, type GeocodedLocation } from '../../integrations/geocoding/geocoding.service';
...
export async function getUserPreferences(
  ...
  notifications.forEach((n) => {
    ...
    if (params?.time) {
      timeCounts.set(params.time, (timeCounts.get(params.time) || 0) + 1);
    }
  });
  const preferredTime =
  ...
  const lastNotification = notifications.find((n) => n.locationId);
  let preferredLocation: GeocodedLocation | null = null;
  if (lastNotification?.locationId) {
    const location = await locationRepo.findById(lastNotification.locationId);
    if (location) {
      preferredLocation = {
        name: location.name,
...
export async function parseNotificationCommand(
  ...
  const normalized = normalizeText(text);
  const preferences = await getUserPreferences(userId, notificationRepo, locationRepo);
